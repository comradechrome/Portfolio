    ...
    public partial class PlayerConsole : Form
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (isRunning)
            {
                Invalidate();
                if (isConnected)
                    Network.Send(worldSocket, "(move, " + this.PointToClient(Cursor.Position).X + ", " + this.PointToClient(Cursor.Position).Y + ")");
            }
        }
        ...
        private double scaleConstant = 3;
        private double locationConstantX;
        private double locationConstantY;
        ...
        private void drawWorld(PaintEventArgs e)
        {
            lock (mainWorld)
            {
                ...
                locationConstantX = mainCube.loc_x; //- (scaleConstant * mainCube.Width / 2);
                locationConstantY = mainCube.loc_y; //- (scaleConstant * mainCube.Width / 2);
                foreach (KeyValuePair<int, Cube> cube in mainWorld.worldCubes)
                {
                    ...
                    Rectangle rectangle = new Rectangle((int)((cube.Value.loc_x - locationConstantX) + mainWorld.worldWidth / 2 ) , 
                                                        (int)((cube.Value.loc_y - locationConstantY) + mainWorld.worldHieght / 2) ,
