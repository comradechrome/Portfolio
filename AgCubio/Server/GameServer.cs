...
namespace Server
{
    class GameServer
    {
        ...
        private static void DBPlayerDeath(string eatenPlayer, string enemyPlayer)
        {
            ...
            //int MaxRank = 2; //TODO: need this to be a counter and passed into method
            ...
            using (MySqlConnection connection = new MySqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    using (MySqlCommand command = connection.CreateCommand())
                    {
                        ...
                        command.Parameters.AddWithValue("@timesInfected", timesInfected);
        ...
        private static void updateCubeStats()
        {
            ...
            foreach (string player in mainWorld.playerCubes.Keys)
            {
                massList.Add(mainWorld.worldCubes[mainWorld.playerCubes[player]].Mass, player);
            }
            int loopCount = 1;
            ...
            foreach(int cubeID in mainWorld.playerCubes.Values)
            {
                mainWorld.ourCubes[cubeID].MaxMass = mainWorld.ourCubes[cubeID].Mass;
            }
        }
                                ...
                                if (otherCube.uid != playerCube.uid)
                                {
                                    ...
                                    // cubes overlap; figure out cube type and take appropriate action
                                    {
                                        ...
                                        else if (otherCube.Mass > playerCube.Mass && !playerCube.food)
                                        {
                                            ...
                                            killPlayer(playerCube, otherCube.Name);
                                            playerCube.Mass = 0;
                                        }
                                        ...
                                        {
                                            playerCube.Mass += otherCube.Mass;
                                            killPlayer(otherCube, playerCube.Name);
                                            otherCube.Mass = 0;
                                        }
                                    }
                                }
        ...
        private static void killPlayer(Cube player, string eaterName)
        {
            ...
            if (player.team_id == 0 || player.team_id == player.uid)
            {
                ...
                DBPlayerDeath(player.Name, eaterName);
